...
if [ -f "${DOCUMENT_ROOT}"/composer.json ]; then
    if [ "$MAGENTO_MODE" = "production" ]; then
        ...
        echo "cd ${DOCUMENT_ROOT} && composer install --prefer-dist --no-interaction --optimize-autoloader 2>&1"
        cd "${DOCUMENT_ROOT}" && composer install --prefer-dist --no-interaction --optimize-autoloader 2>&1

        if [ "${MAGENTO_UPDATE_PACKAGE}" = true ]; then
            echo "composer update --prefer-dist --no-interaction --optimize-autoloader 2>&1"
            composer update --prefer-dist --no-interaction --optimize-autoloader 2>&1
        fi
    fi

    echo "chown -R magento:magento ${DOCUMENT_ROOT}/"
    chown -R magento:magento ${DOCUMENT_ROOT}/
