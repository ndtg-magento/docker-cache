...
production_permission() {
    echo "mkdir -p ${DOCUMENT_ROOT}/var/cache ${DOCUMENT_ROOT}/var/report ${DOCUMENT_ROOT}/var/tmp"
    mkdir -p "${DOCUMENT_ROOT}"/var/cache "${DOCUMENT_ROOT}"/var/report "${DOCUMENT_ROOT}"/var/tmp
    ...
    echo "find ${DOCUMENT_ROOT}/app/code ${DOCUMENT_ROOT}/var/* ${DOCUMENT_ROOT}/vendor \
    ...
    echo "find ${DOCUMENT_ROOT}/var/cache \( -type d -or -type f \) -exec chmod 777 {} +;"
    find "${DOCUMENT_ROOT}"/var/cache \( -type d -or -type f \) -exec chmod 777 {} +;
}